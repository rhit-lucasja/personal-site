...
export const projects = [
    {
        ...
        feature: 'In addition to basic reading and notetaking capabilities, Ignis Divinus features something that physical Bibles lack: a powerful search engine for quickly finding passages on any topic! A post-seeding utility script generates semantic vector embeddings for each verse via Ollama, a local inference point for Nomic AI\'s language processing models. In order to vectorize real-time search queries with the same model, I integrated the Nomic Atlas API in Ignis Divinus\'s backend. As a result, Ignis Divinus efficiently processes search queries through both keyword matching and semantic comparison. The hybrid search engine uses Reciprocal Rank Fusion (RRF) to combine keyword and conceptual similarity for the strongest results.',
        design: 'I built Ignis Divinus from scratch to learn intimately about each step of the software development cycle. An important part of this cycle is continuous integration and continuous deployment (CI/CD). Before diving headfirst into the code, I configured Git branch protection and automated CI/CD workflows via GitHub Actions. CI runs linting and typechecks on non-main pushes and open pull requests, prohibiting merges until all tests pass. Upon merging to main, CD triggers frontend and API deployments, as well as database migrations. With CI/CD, I ensure the quality of any code that reaches production, and I can easily deploy approved changes to the production site!',
        results: 'After two months of initial development, I now use Ignis Divinus nearly every day. I\'m constantly working to improve the app, whether through increased accessibility, additional features, or smoother functionality. At this time, I do not plan to scale the app or campaign for its widespread use, though the site is publicly accessible.',
        tech: ['Next.js', 'TypeScript', 'PostgreSQL + pgvector', 'Auth.js', 'tRPC', 'Ollama', 'Nomic AI', 'Prisma', 'Docker', 'GitHub Actions'],
